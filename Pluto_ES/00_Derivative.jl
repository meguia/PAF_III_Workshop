...
La derivada mide la pendiente instantanea de una funcion. En una recta esa pendiente es constante; en una curva cambia con el punto. En este ejemplo los controles definen un polinomio
$f(x)=a_0+a_1x+a_2x^2+\cdots+a_5x^5$ y el punto $x_0$ indica donde se evalua la pendiente.

La opcion `N` permite mirar derivadas de orden superior: la primera derivada describe cambio, la segunda describe curvatura, y las siguientes describen como cambia esa curvatura.
"""	
...
Mueve los coeficientes para cambiar la forma del polinomio. Observa que cada derivada elimina una potencia y multiplica por el exponente.
"""
...
Elige una funcion de la forma `sin(ax)`, `cos(ax)` o `exp(ax)`. El parametro $a$ controla la rapidez con que cambia la funcion: cuanto mayor es $|a|$, mas grande suele ser la pendiente.
"""
...
Aqui se usa una funcion compuesta, $\cos^2(ax)$, para mostrar que la regla de la cadena aparece naturalmente: primero cambia el argumento $ax$, luego cambia el coseno, y finalmente cambia el cuadrado.
"""
...
limite y : $(Child("ylimit", CheckBox()))
""" 
end;
...
par_widget = @bind par PlutoUI.combine() do Child
	md"""
Mueve el punto donde quieres evaluar la derivada. \
x0 : $(Child("x0", Slider(wind.x1:0.01:wind.x2,default=1.0;show_value=true))) $sp
N : $(Child("N", Slider(0:1:5,default=0;show_value=true)))
tangente : $(Child("tangent", CheckBox()))

La recta tangente es la mejor aproximacion lineal local: cerca de $x_0$, la curva y su tangente casi se confunden.
""" 
end;
...
par_widget2 = @bind par2 PlutoUI.combine() do Child
	md"""
Ajusta el parametro y el punto de evaluacion. \
a : $(Child("a", Slider(-1:0.1:1,default=1.0;show_value=true))) \
x0 : $(Child("x0", Slider(wind.x1:0.01:wind.x2,default=1.0;show_value=true))) $sp
N : $(Child("N", Slider(0:1:5,default=0;show_value=true)))
tangente : $(Child("tangent", CheckBox()))

Cuando $a$ cambia de signo, la funcion invierte la direccion de cambio.
""" 
end;
...
Este caso ayuda a ver la regla de la cadena en una funcion compuesta.
""" 
end;
