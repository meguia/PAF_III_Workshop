...
Elige una funcion de dos variables. El gradiente se evalua en el punto $(x_0,y_0)$.

Para una superficie $f(x,y)$, las derivadas parciales miden cuanto cambia la altura si solo nos movemos en una direccion: $\partial f/\partial x$ mira el cambio horizontal y $\partial f/\partial y$ mira el cambio vertical. El vector gradiente junta esas dos pendientes:

$\nabla f(x_0,y_0)=\left(\frac{\partial f}{\partial x},\frac{\partial f}{\partial y}\right)$

Su direccion apunta hacia el aumento mas rapido de la funcion y su longitud indica que tan fuerte es ese aumento.
"""	
...
gradiente : $(Child("plotg", CheckBox()))
""" 
	end;
elseif npars == 1
	par_widget = @bind par PlutoUI.combine() do Child
...
window_widget = @bind wind PlutoUI.combine() do Child
	md"""
escala : $(Child("scale", Slider(-4:4,default=0;show_value=true))) $sp

La escala solo cambia como se dibujan las flechas; no cambia el gradiente matematico.
""" 
end;
