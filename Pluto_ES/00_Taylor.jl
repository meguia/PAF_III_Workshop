...
Una serie de Taylor aproxima una funcion alrededor de un punto $a$ usando el valor de la funcion y sus derivadas en ese punto. Es como construir una copia local: orden 0 solo usa la altura, orden 1 agrega la pendiente, orden 2 agrega curvatura, y asi sucesivamente.

En un polinomio de grado 5, la expansion de Taylor de orden 5 puede recuperar exactamente el polinomio.
"""	
...
Estos controles definen los coeficientes del polinomio.
"""
...
Elige una funcion usando $x$ como variable. Cerca del punto de expansion $a$, los terminos de Taylor se ajustan muy bien; lejos de $a$, puede hacer falta un orden mayor.
"""
...
par_widget = @bind par PlutoUI.combine() do Child
	md"""
Mueve el punto de expansion y el orden. \
a : $(Child("a", Slider(wind.x1:0.01:wind.x2,default=1.0;show_value=true))) $sp
N : $(Child("N", Slider(0:1:5,default=1;show_value=true)))

El punto $a$ es el centro de la aproximacion. El orden $N$ decide cuantas derivadas entran en el polinomio.
""" 
end;
...
Compara como mejora la aproximacion al aumentar $N$.
""" 
end;
